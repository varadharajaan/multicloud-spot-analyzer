...
"""
        ...
        # Save to file
        output_path = os.path.join(PROJECT_ROOT, "tools", OUTPUT_FILE)
        os.makedirs(os.path.dirname(output_path), exist_ok=True)
        
        with open(output_path, "w", encoding="utf-8") as f:
            f.write(content)
        
        print_success(f"Stack outputs saved to tools/{OUTPUT_FILE}")
...
Examples:
    ...
    if do_deploy:
        print_info(f"Dashboard: Check tools/{OUTPUT_FILE} for URLs")
