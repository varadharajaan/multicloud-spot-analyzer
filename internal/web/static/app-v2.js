    ...
    try {
        ...
        if (data.bestAz) {
            ...
            if (data.equallyRecommended && data.equallyRecommended.length > 1) {
                ...
                // AWS regions: us-east-1, eu-west-2 (digit before zone letter)
                // GCP regions: us-central1, europe-west4 (digit before zone letter but no hyphen in AWS style)
                ...
                // AWS: region format is XX-XXXX-N (e.g., us-east-1, eu-west-2)
                // GCP: region format is XX-XXXXX[N] (e.g., us-central1, europe-west4)
                ...
                if (isAWSFormat) {
                    ...
                    valueSpan.innerHTML = `<strong class="az-link">${regionPrefix}${zoneLetters}</strong>${scoreText}`;
                    ...
                    valueSpan.innerHTML = `<strong class="az-link">${regionPrefix}-${zoneNums}</strong>${scoreText}`;
                }
            } else {
